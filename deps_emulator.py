...
def deps_emulator(folder_name="brown_annotated/"):
    """
    Input: a path to a folder containing all the conllu annotated files by the UDPipe (REST API: https://lindat.mff.cuni.cz/services/udpipe/, the python and R libraries are also available).
    Output:
    ...
    for filename in os.listdir(folder_name):
        if os.path.isfile(folder_name+filename):
            ...
            data = open(folder_name+filename, "r", encoding="utf-8")
            annotations = data.read()
            data.close()
            # Transformation of the conllu file into the accepted format of TreeTagger (c.f. the documentation)
            with open(folder_name+"TOKEN_PER_ROW/token_per_row_"+filename.replace(".conllu", "")+".txt", "w+") as fo:
